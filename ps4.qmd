...
bordering_prefixes = ('870','871','872','873','874','875','876','877','878','879','880','881','882','883','884', '73', '74', '716', '717','718','719','720','721','722','723','724','725','726','727','728','729','700','701','702','703','704','705','706','707','708','709','710','711','712','713','714','715','733', '75', '76', '77', '78', '79')
zips_texas_centroids = zips_all_centroids[zips_all_centroids['ZCTA5'].str.startswith(texas_prefixes)]
zips_texas_borderstates_centroids = zips_all_centroids[zips_all_centroids['ZCTA5'].str.startswith(bordering_prefixes)]
...
zips_withhospital_centroids = zips_texas_borderstates_centroids.merge(hospitals_per_zip_1,on='ZCTA5',how='inner')
```
...
for index, row in subset.iterrows():
    nearest_hospital = zips_withhospital_centroids.distance(row['centroid']).min()
...
The time for calculating the distance for full dataset is around 100 seconds, which is quite closed to the previous estimation.

Referencing stackoverflow for for loop by rows, https://stackoverflow.com/questions/16476924/how-can-i-iterate-over-rows-in-a-pandas-dataframe/55557758#55557758, searching how to do for loop row by row
Referencing https://stackoverflow.com/questions/30740046/calculate-distance-to-nearest-feature-with-geopandas for calculating distance, searching distance to centroid

    c.
```{python}
with open("/Users/boyalin/Documents/GitHub/ppha30538_ps/problem-set-4-boya-zidan/gz_2010_us_860_00_500k/gz_2010_us_860_00_500k.prj", "r") as prj_file:
...
```
    ...
    a.
The average distance is calculated in miles. Therefore, the average distance to the nearest hospital in Texas is 2.962 miles.
    b.
This number makes sense. It represents each zip code area is quite closed to hospital on average, which indicates a good healthcare accessibility in Texas.
    c.
```{python}
texas_distance = zips_texas_centroids.merge(distances_full, on='ZCTA5', how='left')

fig, ax = plt.subplots(1, 1, figsize=(10, 8))
texas_distance.plot(column='distance_miles', ax=ax, legend=True,legend_kwds={'label': "Distance to Nearest Hospital (miles)"})
...
```{python}
texas_closures = corrected_closures[corrected_closures['ZIP_CD'].astype(str).str.startswith(('733', '75', '76', '77', '78', '79'))]

zip_closure_counts = texas_closures.groupby('ZIP_CD').size().reset_index(name='Closures_Counts')

print(zip_closure_counts)

zip_closure_summary = zip_closure_counts['Closures_Counts'].value_counts().reset_index()
zip_closure_summary.columns = ['number_of_closures', 'number_of_zip_codes']
zip_closure_summary
```
...
```{python}
texas_closures['ZIP_CD'] = texas_closures['ZIP_CD'].astype(str)
texas_closures['ZIP_CD'] = texas_closures['ZIP_CD'].str.split('.').str[0].str.strip()

affected_zip_codes = texas_closures['ZIP_CD'].unique()
affected_texas_zips = zips_texas_centroids[zips_texas_centroids['ZCTA5'].isin(affected_zip_codes)]
affected_texas_zips

fig, ax = plt.subplots(1, 1, figsize=(10, 8))
zips_texas_centroids.plot(ax=ax, color='lightgrey',
                          edgecolor='black', linewidth=0.5)
affected_texas_zips.plot(ax=ax, color='red', alpha=0.5)

ax.set_title(
    'Texas ZIP Codes Affected by Hospital Closures (2016-2019)', fontsize=15)
ax.set_xlabel('Longitude')
ax.set_ylabel('Latitude')

plt.show()

print(f'Total affected ZIP codes: {len(affected_zip_codes)}')
```
...
# Step 4: If the CRS is geographic (e.g., EPSG 4326), reproject to a projected CRS (e.g., EPSG 3857)
if zips_texas_centroids.crs is None or zips_texas_centroids.crs == 'EPSG:4326':
    zips_texas_centroids = zips_texas_centroids.to_crs(epsg=3857)

directly_affected_zips['geometry'] = directly_affected_zips.geometry.buffer(buffer_distance)

zips_texas_combined = zips_texas_centroids.merge(distances_full, on='ZCTA5', how='inner')
zips_texas_combined

indirectly_affected_zips = gpd.sjoin(zips_texas_centroids, directly_affected_zips, how='inner', predicate='intersects')

indirectly_affected_zip_codes = indirectly_affected_zips['ZCTA5_left'].unique()

num_indirectly_affected_zips = len(indirectly_affected_zip_codes)
print(f'Total indirectly affected ZIP codes in Texas: {num_indirectly_affected_zips}')
```
...
def categorize_zip(zip_code):
    if zip_code in directly_affected_zips:
        return 'Directly Affected'
    elif zip_code in indirectly_affected_zip_codes:
        return 'Indirectly Affected'
    else:
        return 'Not Affected'

zips_texas_centroids['category'] = zips_texas_centroids['ZCTA5'].apply(categorize_zip)
...
zips_texas_centroids['color'] = zips_texas_centroids['category'].map(color_map)

fig, ax = plt.subplots(1, 1, figsize=(10, 8))

zips_texas_centroids.plot(ax=ax, color=zips_texas_centroids['color'], edgecolor='black', linewidth=0.5)
