...
```{python}
#taking active hospitals datas and group by on zipcode
...
```{python}
#merge number of active hospitals grouped by zipcode to one dataset
...
#get the number of unique zipcode
...
3. 
```{python}
# filter zipcode with at least one hospital
